...
import { DBNAME_MAIN, DBTABLE_USER } from '../database_constants';
...
export async function getUser(username: string): Promise<UserProps | null> {
  ...
  const collection = client.db(DBNAME_MAIN).collection(DBTABLE_USER);
...
export async function getAllUsers(): Promise<ResultProps[]> {
  const client = await clientPromise;
  // console.log('getAllUsers client', client);
  const collection = client.db(DBNAME_MAIN).collection(DBTABLE_USER);
  // console.log('getAllUsers collection', collection);
